...
-(void)audioStream:(TDAudioStream *)audioStream didRaiseOutputEvent:(TDAudioStreamEvent)event {
    NSLog(@"Ich soll niemals aufgerufen werden, weil ich in AudioInputStreamer bin...");
}
...
- (void)stopStreaming
{
    [self performSelector:@selector(stopInputThread) onThread:self.audioStreamerThread withObject:nil waitUntilDone:YES];
}
