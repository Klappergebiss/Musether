...
- (void)stop
{
    [self performSelector:@selector(stopOutputThread) onThread:self.streamThread withObject:nil waitUntilDone:YES];
}
...
- (NSStreamStatus) streamStatus {
    return [self.audioStream streamStatus];
}
...
-(void)audioStream:(TDAudioStream *)audioStream didRaiseInputEvent:(TDAudioStreamEvent)event {
    NSLog(@"Ich soll niemals aufgerufen werden, weil ich in AudioOutputStreamer bin...");
}
